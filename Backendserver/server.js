...
app.post('/gallery', (req, res) => {
    ...
    data.insert({ email: email, image: image, description: description }).then((resui) => console.log("ok"));
})

app.post('/getgallery', (req, res) => {
    const { email } = req.body;
    const data = db.get('gallery');
    data.find({ email }).then((val) => { 
        if(val.length == 0)
            res.send(['no'])
        else
            res.send(val) 
...
app.post('/deleteGalleryItem',(req,res)=>{
    const { id } = req.body;
    const data = db.get('gallery');
    data.remove({ _id: ObjectId(id) }).then((result) => console.log(result))
})
///
